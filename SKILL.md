---
name: content-hunter
description: 内容捕手 (Content Hunter) - 短视频平台热门内容抓取机器人。自动刷小红书、抖音，B站、快手4个平台，每次每平台抓取20条热门内容并保存，晚上8点生成汇报。/Content Hunter - Hot content crawler for Xiaohongshu, Douyin, Bilibili, Kuaishou. Scrapes 20 items per platform and generates daily report at 8 PM.
---

# 内容捕手 / Content Hunter

## 概述 / Overview
持续刷小红书、抖音，B站、快手4个平台，每次每平台抓取20条热门内容并保存，晚上8点生成汇报。/Scrapes 4 platforms (Xiaohongshu, Douyin, Bilibili, Kuaishou), 20 items each, generates daily report at 8 PM.
...
| 快手 / Kuaishou | https://www.kuaishou.com | 标题、作者、播放、点赞、内容总结 / Title, Author, Views, Likes, Summary | 需优化加载 / Needs optimization |
...
### 4.4 B站字幕提取 / Bilibili Subtitle Extraction
```bash
python3 ~/.openclaw/workspace/skills/bilibili-youtube-watcher/scripts/get_transcript.py "视频URL" --lang zh-CN
```

### 4.5 快手优化 / Kuaishou Optimization
- 快手网页版是单列视频流，一次只显示1个视频 / Kuaishou web is single-column video feed, shows 1 video at a time
- 需要多次滚动+等待才能获取更多内容 / Need multiple scrolls + waits to get more content
- 每次滚动后等待3-5秒让内容加载 / Wait 3-5 seconds after each scroll for content to load
- **必须滚动至少20次以获取20条内容** / **Must scroll at least 20 times to get 20 items**
- 每次滚动操作：browser.act({key: "End"}) + sleep(3-5) / Each scroll: press End key + wait
- 如果页面内容重复，刷新页面重新滚动 / If content repeats, refresh and scroll again
- 最多尝试3次 / Max 3 attempts
...
| B站 / Bilibili | ✅ |
| 快手 / Kuaishou | ✅ |
| 微博 / Weibo | ❌ 需登录 / Login required |
| 知乎 / Zhihu | ❌ 需登录 / Login required |
| 小宇宙 / Xiaoyuzhou | ❌ 需登录 / Login required |
...
openclaw cron add --name "内容捕手-抓取" --cron "*/30 9-20 * * *" --session isolated --message "请执行内容捕手抓取：每次每平台抓20条，追加保存到 ~/.openclaw/workspace/content-hunter/data/ 对应md文件中（是追加不是覆盖！）"

# 晚上8点汇报 / Daily report at 8 PM
openclaw cron add --name "内容捕手-汇报" --cron "0 20 * * *" --session isolated --message "请执行内容捕手汇报：读取数据生成汇报发送到群里"
```
...
|------|------|--------|----------|
| 平台 / Platforms | 要抓取的平台 / Platforms to scrape | 全部4个 / All 4 | "只刷抖音" / "Only Douyin" |
| 数量 / Count | 每平台抓取条数 / Items per platform | 20条 / 20 | "每个刷30条" / "30 items each" |
...
└── task-2026-03-11-2100/           # 任务文件夹（按时间命名）
    ├── xiaohongshu.md              # 小红书数据（20条，每条含标题、作者、点赞、内容总结）
    ├── douyin.md                    # 抖音数据
    ├── bilibili.md                 # B站数据
    ├── kuaishou.md                # 快手数据
    └── summary.md                  # 汇报文件
...
```
用户A：帮我刷短视频平台
→ AI：好的，启动内容捕手，默认抓取4个平台，每平台20条
/User A: Scrape video platforms
→ AI: OK, starting Content Hunter, 4 platforms, 20 items each
